...
export default function TabelasPrecosConfigManager() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const sucesso = await response.json(); // Assume que a API retorna { sucesso: true } ou { sucesso: false }
        ...
        if (sucesso) {
          ...
          console.log('🔄 Sincronizando IndexedDB após salvar configuração...')
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (sucesso) {
          ...
          // Sincronizar IndexedDB
          console.log('🔄 Sincronizando IndexedDB após desativar configuração...')
          const prefetchRes = await fetch('/api/prefetch', { method: 'POST' })
          if (prefetchRes.ok) {
